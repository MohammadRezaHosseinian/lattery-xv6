...
scheduler(void)
{
  ...
  for(;;){
    ...
    int lotteryWin = randomrange(0, totalTickets);
    for(p = proc; p < &proc[NPROC]; p++) {
        lotteryWin -= p->tickets; // #EDITED
        if (lotteryWin >0)// #EDITED
            continue;// #EDITED
        acquire(&p->lock);
      ...
      if(p->state == RUNNABLE) {
        ...
        p->state = RUNNING;
        c->proc = p;
        int startTicks = ticks; // #EDITED

        swtch(&c->context, &p->context);

        p->ticks += ticks - startTicks; // #EDITED
...
int setTickets(int n){
    ...
    // find process and set ticket value with n
    struct proc *p;
    p = myproc();
    p->tickets = n;
    return 0;
